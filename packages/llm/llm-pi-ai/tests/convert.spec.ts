...
describe('mapStopReason / mapUsage', () => {
  ...
  it('maps routable HTTP-ish error messages to stable codes', () => {
    ...
    expect(mapStopReason(assistant({ stopReason: 'error', errorMessage: 'Error Code server_is_overloaded' })))
      .toMatchObject({ kind: 'error', failure: { code: 'SERVER' } })
    expect(mapStopReason(assistant({ stopReason: 'error', errorMessage: 'Error Code server_error: An error occurred while processing your request. Please include the request ID 5ebc46cf-0fd4-4298-bae7-af5109b14994.' })))
      .toMatchObject({ kind: 'error', failure: { code: 'SERVER' } })
    expect(mapStopReason(assistant({ stopReason: 'error', errorMessage: 'Our servers are currently overloaded. Please try again later.' })))
      .toMatchObject({ kind: 'error', failure: { code: 'SERVER' } })
