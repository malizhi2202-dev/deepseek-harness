...
import { SOURCES_ID, SOURCES_KIND } from '../src/client/definition.ts'
...
async function boot() {
  ...
  const sources = { status: vi.fn(), probe: vi.fn() }
  const settings = { describe: vi.fn() }
  const remote = { sources, settings, $stream: vi.fn() }
  const settingsScope = {
  ...
  ctx.provide('remote.sources', sources as never)
...
describe('ui-sidebar-sources apply', () => {
  ...
  it('registers the type, its dictionaries, and the body seat under the type\'s id with a store and a face', async () => {
    ...
    const definition = tabs.get(SOURCES_KIND)
    expect(definition?.id).toBe(SOURCES_ID)
    expect(definition?.priority).toBe('builtin')
    expect(definition?.title('sources')).toBe('type.label')
    expect(definition?.guide?.map(entry => [entry.order, entry.title(), entry.description()])).toEqual([
      [70, 'guide.title', 'guide.description'],
    ])
    expect(dictionaries.get('sidebarSources')).toEqual({ zh, en })
    ...
    expect(registered.map(entry => [entry.name, entry.key, entry.locale, entry.component])).toEqual([
      ['sidebar.right.pane.tab', SOURCES_ID, 'sidebarSources', SourcesBody],
    ])
  ...
  it('takes every registration back when the plugin is disposed', async () => {
    ...
    await fiber.dispose()
    expect(tabs.get(SOURCES_KIND)).toBeUndefined()
