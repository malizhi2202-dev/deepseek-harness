 ...
 * handed to them — one body seat under the type's id, and the two dictionaries
 * — and that every registration is gone after dispose, which is what makes a
 * reload safe.
 */
import { describe, expect, it, vi } from 'vitest'
import { Context } from '@deepseek-ai/cordis'
import { SidebarRightTabRegistry } from '@deepseek-ai/dsh-client-ui-sidebar-right/src/client/tab-registry.ts'
import { TASKS_ID, TASKS_KIND } from '../src/client/definition.ts'
...
describe('ui-sidebar-tasks apply', () => {
  ...
  it('registers the tasks type, its dictionaries, and the body seat under the type\'s id', async () => {
    ...
    expect(definition?.title('sidebar://tasks')).toBe('type.label')
  ...
  it('takes every registration back when the plugin is disposed', async () => {
    ...
    expect(dictionaries.has('sidebarTasks')).toBe(false)
  })
})
