...
import { TuituiCard } from './TuituiCard.tsx'
...
import { TUITION_NS, TuituiCardController } from './tuitui-card-controller.ts'
...
export type { TuituiCardFace, TuituiCardState } from './tuitui-card-controller.ts'
...
export function apply(ctx: ClientContext): void {
  ...
  const tuitui = new TuituiCardController(
    ctx.settingsScope.bind({ namespace: TUITION_NS }), ctx.settingsScope.describe())
  ...
  // The tuitui card follows the shared mirror for its redacted secret slot, so
  // its subscription must leave with the plugin fiber.
  ctx.effect(() => () => { tuitui.dispose() }, 'ui-settings-plugins: tuitui card')
