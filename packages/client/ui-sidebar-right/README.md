...
The strip's other entry into the column is the type picker: a button in the panel's chrome that lists the page types the registry holds, skipping the guide (the add control opens that) and any `hidden` type, in ascending `order` and with each type's declared `icon`. Picking a row calls `openTab(kind, { paneId })` for the pane the button sits in, which is how a type a surface did not open stays reachable.
...
- **The default-visible budget is a constant.** `MAX_DEFAULT_VISIBLE_TABS` is fixed in `contract/visibility.ts` and read by the registry and the gate; no `Config` field or setting raises it, so a deployment cannot decide how many tabs a surface opens by itself.
