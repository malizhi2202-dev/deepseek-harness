...
新停靠面是一个格子，里面装着引导页，以及注册表报告为 `default-on` 的每个类型一个 tab，按 `order` 升序排列，并聚焦引导页。这些 tab 属于初始布局、不属于任何记录条目，因此回退止步于会话诞生时的样子，关掉其中一个也不会被随后的 settle 撤销。注册表的默认开启集由 store 的 seed（`SurfaceSeed`）为每个新停靠面重新读取，因此会话打开之前注册的类型会被种入该会话。
