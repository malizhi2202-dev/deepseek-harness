 ...
 * makes a reload safe.
 *
 * The injected service is the shared `remote.workspace` namespace: this panel's
 * ledger read lives beside the Workspace Controller's methods, so the panel
 * declares that service and no namespace of its own.
 */
...
async function boot() {
  ...
  const workspace = { automationLedger: vi.fn() }
  ctx.provide('sidebarRightTabs', tabs as never)
  ctx.provide('slots', slots as never)
  ctx.provide('locale', locale as never)
  ctx.provide('remote', { workspace } as never)
  ctx.provide('remote.workspace', workspace as never)
  const fiber = ctx.plugin({ inject: [...inject], apply })
  await fiber.await()
  return { tabs, registered, dictionaries, fiber }
}

describe('ui-sidebar-automation apply', () => {
  ...
  it('registers the type, its dictionaries, and the body seat under the type\'s id with a store and a face', async () => {
    ...
    const definition = tabs.get(AUTOMATION_KIND)
    expect(definition?.id).toBe(AUTOMATION_ID)
    expect(definition?.priority).toBe('builtin')
    expect(definition?.title('automation')).toBe('type.label')
    expect(definition?.guide?.map(entry => [entry.order, entry.title(), entry.description()])).toEqual([[80, 'guide.title', 'guide.description']])
    expect(dictionaries.get('sidebarAutomation')).toEqual({ zh, en })
    ...
    expect(registered.map(entry => [entry.name, entry.key, entry.locale, entry.component])).toEqual([
      ['sidebar.right.pane.tab', AUTOMATION_ID, 'sidebarAutomation', AutomationBody],
    ])
  ...
  it('takes every registration back when the plugin is disposed', async () => {
    ...
    await fiber.dispose()
    expect(tabs.get(AUTOMATION_KIND)).toBeUndefined()
