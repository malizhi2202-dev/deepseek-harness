---
description: "The right Sidebar's plain-text viewer tab type for the dsh web client: paged reads of one workspace file, line navigation, wrap, reload, and guarded whole-file editing, with the fallback claim on every file resource address."
...
- **The body** — the keyed `sidebar.right.pane.tab` seat under the type's id. Its header row shows the Host's absolute path and exposes its full value in a tooltip, with the type's controls at its end: an edit button when the file can be edited, a wrap toggle (on by default; long lines wrap until the reader turns it off, per tab), and a reload button; while editing, save and cancel take their place. The Sidebar's tab strip carries no controls of this type. The body takes the pane body's full height: the header row stays put and the file body below is the one scroller, so a short file leaves no unstyled space and a long file scrolls under a fixed path. Until metadata provides an absolute path, the header uses the requested path.
- **One store and one face**, session-scoped and bucketed by tab id. The store holds the pages read so far (keyed by the 1-based line each starts at, with the file version and byte size they belong to), the end-of-file flag, the read in flight or its failure, the edit in progress with the state of its save, and the view: scroll offset, wrap (initially on), and the navigation revision the body last answered. The face (`loadPage`, `reloadPages`, `save`) performs the reads and the write through the store's actions. The bucket is forgotten when the owner's `signal` aborts, which is when the tab record is gone.
...
- **Reload** — the change bar's button and the header's reload control both call the resource's `reload()` (a fresh `stat`, which clears `changed`) and the face's `reloadPages` (drop the pages, read the first one again). A reload retires the reads still in flight — the face keeps a request generation per tab, and a page settling from an older generation writes nothing. The scroll offset is kept, so the reader stays where they were. A reload also drops an edit in progress: the pages it brings back are a different file version, so the draft's save would be refused anyway.
...
An edit replaces the whole file, so the viewer offers one only for a file it holds whole, and the save carries the version its text was read at.

- **When the editor opens.** The header shows an edit button exactly when the tab holds one page, starting at line 1, reaching the end of the file, with a known version and neither a read in flight nor a failed one. Everything else shows a sentence naming the reason instead — still reading, the read failed, or the file is read in several pages — and no edit button. A partially read file is refused rather than completed on demand: the viewer cannot write back lines it never loaded, and loading the rest first would make the button's meaning depend on a read that may fail.
- **The text.** A page joins its lines with `\n` and drops the terminator of the last one, so the page text alone cannot tell a file ending in a newline from one that does not. `fullText` compares the page's own UTF-8 size with the byte size the Host reported for the file: equal means no dropped terminator, unequal means one newline is put back. With no reported size the text stands as read. `TextEncoder` measures it because `Buffer` does not exist in the browser.
- **The draft.** Opening the editor records a draft — the text as read (the baseline), the text as typed, and the version — in the tab's store bucket. Typing rewrites the text and returns the save to idle; a save with no change from the baseline is not offered. Cancel drops the draft and leaves the pages alone.
- **The save.** Save calls `remote.workspaceFiles.write(sessionId, path, text, version, signal)`, where `version` is the version the draft's text was read at, not the version on screen now. While it is in flight the button is disabled; on success the written text becomes the pages at the new version and byte size, the editor closes, and the header says **Saved**; on failure the draft is kept exactly as typed and the refusal is stated — a stale version ("the file changed on disk, so your edit was not saved") with a reload button beside it, or an oversized write with the byte cap. The refusal never discards the reader's text.
- **A change under the editor.** The change bar and the metadata-failure bar keep their place while editing, and either one's reload drops the draft along with the pages. The save's own version guard is what catches a concurrent write the Host has not reported yet.

The editor is a `<textarea>` that takes the file body's place, so the file body is unmounted while it is open: a navigation that arrives then is answered when the edit closes, and the scroll offset is restored then too. The save is the only mutation, and it goes through the Remote method: the browser never touches the filesystem. The save passes the session's sandbox policy with the write, so a session that may not write the workspace cannot save either.
...
- **Whole-file editing only.** A file read in several pages cannot be edited here at all, and an edit always rewrites the file from its first byte; there is no editing of a page, an offset range, or a line.
- **No unsaved-changes guard.** Closing a tab or switching away with a draft in the editor discards it without asking; the tab record's abort is what ends the draft.
