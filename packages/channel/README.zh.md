...
`channel/` 包族让聊天平台驱动一个 Agent Session，并把该 Session 已完成的回复镜像回会话。`channel` 是服务定义：连接器注册表、一条携带惰性媒体句柄的规范化入站消息，以及提供方实现的出站客户端。`channel-bridge` 是所有平台共用的消费方，它拥有必须在各处保持一致的决策——去重、会话锁定、经 Agent 注册表接纳、出站转发，以及哪些回复文件可以离开工作区。`channel-tuitui` 是适配本仓库既有传输的提供方。`api-channels` 通过 Typert Remote 暴露状态与启用/停用绑定。
