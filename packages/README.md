...
| [`git/`](git/README.md) | Read-only repository observation: seam, local provider, the Web git panel it feeds |
...
| [`resource/`](resource/README.md) | Remote resource family: the `ctx.sources` seam, wiki/code/database providers, model-facing source tools |
