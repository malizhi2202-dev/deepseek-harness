...
| [`client/ui-sidebar-textpreview`](../../packages/client/ui-sidebar-textpreview/README.md), [`client/ui-sidebar-files`](../../packages/client/ui-sidebar-files/README.md), [`client/ui-sidebar-tasks`](../../packages/client/ui-sidebar-tasks/README.md), [`client/ui-sidebar-agents`](../../packages/client/ui-sidebar-agents/README.md), [`client/ui-sidebar-git`](../../packages/client/ui-sidebar-git/README.md), [`client/ui-sidebar-terminal`](../../packages/client/ui-sidebar-terminal/README.md), [`client/ui-sidebar-channels`](../../packages/client/ui-sidebar-channels/README.md) | The shipped `text`, `files`, `tasks`, `agents`, `git`, `terminal`, and `channels` types |
...
| `kind` | The type's discriminator: what its tabs are, and what `openTab` names. Not unique — an extension may take over a builtin's kind. The shipped kinds are `guide`, `text`, `files`, `tasks`, `agents`, and `git`. |
...
- **`git`** — `builtin`, `available`, order 400, reached from the guide and opened as `openTab('git')`. The session workspace's repository, observed through the Host's `workspaceGit` Remote namespace: the head's facts, the local branch list, the bounded history with its lane gutter, and the worktree's entries. It never writes to the repository, and a workspace outside one says so in a line ([README](../../packages/client/ui-sidebar-git/README.md)).
- **`terminal`** — `builtin`, `available`, order 500, reached from the guide and opened as `openTab('terminal')`. The session's own shell console, driven through the Host's `terminalConsole` Remote namespace: the shells the session holds, the one on screen, and the bounded sanitized text it has printed. It is the person's channel, not the model's — nothing it prints enters the session log — and an install serving a surface a network can reach refuses it by default ([README](../../packages/client/ui-sidebar-terminal/README.md)).
- **`channels`** — `builtin`, `available`, order 600, reached from the guide and opened as `openTab('channels')`. Every chat channel the Host's connector registry serves, driven through the Host's `channels` Remote namespace and read through the settings scope: the connection each reports, its credential references by name, its declared capabilities, and a settings form built from the channel's own configuration schema. Enabling binds a channel to the tab's own session; the panel never reads or shows a credential value ([README](../../packages/client/ui-sidebar-channels/README.md)).
