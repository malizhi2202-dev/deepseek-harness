...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/channel/channel-feishu': { kind: 'indirect', reason: 'The provider normalizes Feishu events into the seam\'s inbound type and renders outbound text; the admitting bridge owns every model-visible effect.' },
  'packages/channel/channel-dingtalk': { kind: 'indirect', reason: 'The provider normalizes DingTalk robot callbacks into the seam\'s inbound type and renders outbound text; the admitting bridge owns every model-visible effect.' },
  'packages/channel/channel-qq': { kind: 'indirect', reason: 'The provider normalizes QQ group and single-chat events into the seam\'s inbound type and renders outbound text; the admitting bridge owns every model-visible effect.' },
  'packages/channel/channel-wechat': { kind: 'indirect', reason: 'The provider normalizes WeChat bot-channel messages into the seam\'s inbound type and renders outbound text; the admitting bridge owns every model-visible effect.' },
  ...
  'packages/client/ui-sidebar-tasks': { kind: 'none', reason: 'Browser-only task panel reading an existing projection and job mirror; registers no tool, prompt section, or session event.' },
  ...
  'packages/api/terminal-console': { kind: 'none', reason: 'Client-facing Remote namespace over the PTY registry; registers no tool, prompt section, or session event, and nothing it carries reaches a model request.' },
