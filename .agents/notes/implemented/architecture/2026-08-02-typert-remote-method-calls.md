...
| Connection | `ctx.connection` | Exclusively owns the HTTP Server and its WebSocket upgrade path (the Gateway serves its Remote stream route there), the shared `/api` route, RPC envelope, rpcId, serialization, trust, error transport, Typert interception, and owner-registered exact Fetch routes on the same channel |
...
- Connection: owns the single HTTP Server and the WebSocket upgrade path on it, the shared `/api` route and its composite FetchHandler, owner-registered exact Fetch routes, the RPC envelope, rpcId, serialization, trust, and error transport.
