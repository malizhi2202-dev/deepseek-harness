...
| 3 | git-graph 上游修复状态 | **有界代价评估已完成：无法在有界范围内判定——代价面有界且闭合，卡点在证据不在代价**。**负责人已裁定走 B（核心自建）**：留在当前线新造能力缝插件，运行时以工作区是否为 git 仓库为准——**有 git 则用工作区 git，没有则面板不可用**，并显式标注「有期限的临时物」 | 四项前置 ①已满足、②③④未满足（②需整份副本验证、③迁移源谱系本地不可判、④免费/自建清单未出具）；翻转条件与详情见下方「第 3 项评估记录」。**A（迁移后用上游）保持为迁移后的长期路径**，其人为前置不变；树内无任何既有 git 调用（已核），自建须从零新造 Service Definition / Provider / Consumer 三角 |
